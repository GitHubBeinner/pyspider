...
class Fetcher(object):
    ...
    def chrome_fetch(self, url, task):
        '''Fetch with chrome proxy'''
        start_time = time.time()
        self.on_fetch('chrome', task)
        handle_error = lambda x: self.handle_error('chrome', url, task, start_time, x)

        # check chrome proxy is enabled
        if not self.chrome_proxy:
            result = {
                "orig_url": url,
                "content": "chrome is not enabled.",
                "headers": {},
                "status_code": 501,
                "url": url,
                "time": time.time() - start_time,
        ...
        try:
            request = tornado.httpclient.HTTPRequest(
                url=self.chrome_proxy, method="POST",
                body=json.dumps(fetch), **request_conf)
        except Exception as e:
            raise gen.Return(handle_error(e))
